...
export default function RegistrarScreen(P) {
  ...
  useEffect(() => {
    ...
    const linAct = P.lineas?.find(l => l.id === P.lid)
    const origenAct = P.oris?.find(o => o.id === parseInt(f.origen_id))
    const pts = []
      ...
      <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.97)',zIndex:9500,display:'flex',flexDirection:'column'}}>
        ...
        <div style={{padding:'12px 16px 20px',background:'rgba(0,0,0,0.5)',display:'flex',gap:8,flexWrap:'wrap'}}>
          {hayMejorada ? (
            ...
            <>
              {!foto.es_principal && (
                <button onClick={()=>{marcarPrincipal(fotoViewer);notify('⭐ Marcada como principal')}}
                  style={{flex:1,padding:'12px 0',borderRadius:10,border:'none',background:G.gold,color:'#fff',fontSize:13,fontWeight:700,cursor:'pointer'}}>
                  ⭐ Principal
                </button>
              )}
              {emp?.api_openai_key && (
